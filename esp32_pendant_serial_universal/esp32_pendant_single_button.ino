/*
  UNIVERSAL AIONMECH CNC PENDANT – ESP32 WROOM SINGLE BUTTON VERSION
  ...
  - Hold 2s: Change Machine Type
  - 6 clicks: Toggle WCS/MCS View
  
  Wiring:
    Encoder A : GPIO32 (interrupt, rising)
    Encoder B : GPIO33
    Button    : GPIO25 (to GND, INPUT_PULLUP, active-low) - Single Control Button
...
#define PIN_BUTTON      25  // Single control button
#define PIN_TOGGLE_SW   27  // Power/Sleep
...
// ================ Globals ================
#define FW_VERSION "ULP-ESP32 v2.0.0-SINGLE-BUTTON"
...
const unsigned long LONG_PRESS_THRESHOLD = 2000;  // 2 seconds for machine change
const unsigned long DEBOUNCE_DELAY = 50;
const unsigned long MULTI_PRESS_WINDOW = 800;     // 800ms window for multi-press detection
...
void drawBootScreen(){
  ...
  lcd.setTextColor(getMachineAccentColor(),0x0000);
  lcd.setFont(&fonts::Font2); lcd.drawString("SINGLE BUTTON",120,120);
  ...
  lcd.drawString("ENC: Jog | BTN: 1x Axis 2x Dist 3x Speed",120,170);
  lcd.drawString("Hold 2s: Machine | 6x: W/M view",120,184);
  UI_End();
}
...
void drawHelpScreen(){
  ...
  lcd.setFont(&fonts::Font4); lcd.drawString("SINGLE BUTTON",120,28);
  ...
  lcd.setTextColor(getMachineAccentColor(), THEME_BG_DARK);
  lcd.drawString("BUTTON:",18,90);
  lcd.setTextColor(THEME_TEXT_MAIN, THEME_BG_DARK);
  lcd.drawString("1x = Change Axis",18,104);
  lcd.drawString("2x = Change Distance",18,118);
  lcd.drawString("3x = Speed/Feed",18,132);
  lcd.drawString("4x = Theme",18,146);
  lcd.drawString("5x+ = Help",18,160);
  lcd.drawString("HOLD 2s = Machine",18,174);
  lcd.drawString("6x = Toggle WCS/MCS",18,188);
...
void handleSingleButton(){
  ...
  if((now - dbT) > DEBOUNCE_DELAY){
    ...
    else if(!down && buttonPressed){
      buttonPressed = false;
      unsigned long dur = now - buttonPressTime;
      
      if(!buttonProcessed){
        if(dur >= LONG_PRESS_THRESHOLD){
          // Long press: Change machine type
          currentMachine = (MachineType)((currentMachine + 1) % 9);
          switch(currentMachine){
            case MACHINE_FIRECONTROL:
            case MACHINE_CUTCONTROL:
              currentProtocol = PROTOCOL_LANGMUIR; break;
            case MACHINE_MACH3:
            case MACHINE_MACH4:
            case MACHINE_UCCNC:
              currentProtocol = PROTOCOL_MACH; break;
            case MACHINE_LINUXCNC:
              currentProtocol = PROTOCOL_LINUXCNC; break;
            case MACHINE_CARBIDE:
            case MACHINE_UGS:
              currentProtocol = PROTOCOL_GRBL; break;
            default:
              currentProtocol = PROTOCOL_LANGMUIR; break;
          }
          Serial.print("MACHINE:"); Serial.println(getMachineName());
          Serial.print("PROTOCOL:"); Serial.println(currentProtocol);
          UI_Begin(); drawBackground(); drawStatusStaticRings(); drawAxisDisplay(); drawSpeedIndicator(); drawConnectionStatus(); UI_End();
          buttonProcessed = true;
        } else {
          // Short press: increment counter for multi-press detection
          buttonPressCount++;
    ...
    if(buttonPressCount > 0 && (now - lastMultiPressTime) > MULTI_PRESS_WINDOW){
      UI_Begin();
      ...
      if(buttonPressCount == 1){
        ...
        // 4 clicks: Change theme
        currentThemeVariant = (currentThemeVariant + 1) % 3;
        Serial.printf("THEME:VARIANT,%d\n", currentThemeVariant);
        drawAxisDisplay(); drawSpeedIndicator(); drawConnectionStatus();
        
      } else if(buttonPressCount == 6 && !viewLockedByHost){
        // 6 clicks: Toggle WCS/MCS view
        coordView = (coordView == VIEW_WCS) ? VIEW_MCS : VIEW_WCS;
        Serial.printf("COORD:VIEW,%s\n", coordView == VIEW_WCS ? "WCS" : "MCS");
        drawAxisDisplay(); drawConnectionStatus();
...
void setup(){
  ...
  pinMode(PIN_BUTTON,INPUT_PULLUP); pinMode(PIN_TOGGLE_SW,INPUT_PULLUP);
  ...
  Serial.printf("FW:%s\n", FW_VERSION);
  Serial.print("MACHINE:"); Serial.println(getMachineName());
  Serial.println("AIONMECH Single Button CNC Pendant starting...");
  Serial.println("PENDANT:READY");
  Serial.println("FIRMWARE:SINGLE_BUTTON_V2.0");
}
